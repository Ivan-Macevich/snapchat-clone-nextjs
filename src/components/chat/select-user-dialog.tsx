...
import { TextMessageSent } from "src/svgs/chatSvg";
import UserCard from "./user-card";
import { sendMessageAction } from "src/lib/actions";
...
const SelectUserDialog = ({ selectedFile, onClose, onPrev }: SelectUserDialogProps) => {
	...
	const handleSendMessage = async () => {
		...
		try {
			await sendMessageAction(selectedUser?._id as string, selectedFile, "image");
			...
			>
				<DialogHeader>
					...
					{users.map((user: IUserDocument) => (
						<div key={user._id as string} className='flex flex-col max-h-48 bg-sigSurface rounded-md overflow-auto'>
